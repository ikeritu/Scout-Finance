...
def main() -> int:
    ...
    assert sum(r["batch_eligible"] == "true" for r in rows) == 738 + overlay_count
    assert Counter(r["acquisition_status"] for r in rows) == {
        "READY_FOR_CONTROLLED_BATCH": 738 + overlay_count, "SYMBOL_RESOLUTION_REQUIRED": 3659 - overlay_count, "USER_ACTION_REQUIRED": 5011,
    ...
    assert sum(r["exchange"] == "JPX" and bool(r["provider_symbol"]) for r in rows) == 42 + overlay_count
    print(f"PASS: v2.38B/contract/43089/21165/{738 + overlay_count}-ready/{3659 - overlay_count}-symbol-resolution/closed-states/no-phase9c")
    return 0
