...
def main() -> int:
    ...
    assert sum(r["batch_eligible"] == "true" for r in rows) == 738
    assert Counter(r["acquisition_status"] for r in rows) == {
        "READY_FOR_CONTROLLED_BATCH": 738, "SYMBOL_RESOLUTION_REQUIRED": 3659, "USER_ACTION_REQUIRED": 5011,
    ...
    assert sum(r["exchange"] == "JPX" and bool(r["provider_symbol"]) for r in rows) == 42
    print("PASS: v2.38B/contract/43089/21165/738-ready/3659-symbol-resolution/closed-states/no-phase9c")
    return 0
