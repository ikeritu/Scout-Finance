...
def main() -> int:
    ...
    assert manifest["eligibility_preflight"]["v2_33b_reopen_required"] is False
    assert not any(row["price_collection_status"] == "blocked_pending_eligibility_correction" for row in rows)
    ...
    print("PASS: v2.33D-preparation/240-unique/refined-eligibility-preflight-clean/fail-closed/no-ranking")
    return 0
