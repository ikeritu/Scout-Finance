...
def test_unicorns_has_dedicated_renderer_and_dispatch() -> None:
    ...
    require("def render_featured_explosive_candlestick(" in source, "Explosive dashboard must show a visible featured candlestick panel")
    ...
    require("Velas del candidato destacado" in source, "Candlestick chart must be visible above cards, not only hidden in expanders")
    require("f\"{identifier}.csv\"" in source, "Candlestick loader must support both asset_id and ticker local files")
    ...
    require("def stable_explosive_diversity_key(" in source, "Explosive candidate tie-breaks must use stable diversified ordering")
    require("def explosive_dashboard_sort_key(" in source, "Explosive dashboard must centralize its sort key")
    require("desempate diversificado estable, no alfabético" in source, "Explosive dashboard must disclose non-alphabetical tie-breaks")
    require("key=explosive_dashboard_sort_key" in source, "Explosive score views must use diversified tie-break sorting")
    require("(-int(row.get(\"explosive_score_0_100\", 0)), row.get(\"company_name\", \"\"))" not in source, "Explosive score sorting must not fall back to company alphabet")
    require("high_conviction_signals" in source, "Explosive score must count strong signals explicitly")
    require("score = min(score, 59)" in source, "Explosive score must cap incomplete candidates below high tiers")
    require("score >= 88 and high_conviction_signals >= 5 and breakout" in source, "Explosive HIGH tier must require strict confirmation")
    require("score >= 72 and high_conviction_signals >= 4" in source, "Explosive MEDIUM tier must require multiple strong signals")
    require("def render_explosive_trading_desk_skin(" in source, "Explosive view must provide a custom trading-desk visual skin")
    require("def render_explosive_trading_desk_hero(" in source, "Explosive view must render a dedicated trading-desk hero")
    require("sf-trading-hero" in source and "sf-radar" in source, "Explosive hero must include dark trading-desk radar styling")
    require("Top 3 candidatos calientes" in source, "Explosive hero must lead with top hot candidates")
    require("Filtros de exploración y métricas técnicas" in source, "Explosive filters must be secondary and collapsible")
    require("@keyframes sfRadarSweep" in source and "@keyframes sfHotPulse" in source, "Explosive hero must include professional motion cues")
