#!/usr/bin/env python3
"""Offline QA for the v2.38W accounts document fetch runner. No real
...
SCRIPT = ROOT / "scripts/fetch_europe_accounts_documents_v2_38w.py"
...
def test_dry_run_reports_resolved_assets_without_network():
    mod = module("fetch_w_1")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        matrix = root / "matrix.csv"
        write_lookup_matrix(matrix, [{"asset_id": "U1", "ticker": "SCT", "resolved_company_name": "SOFTCAT PLC", "company_number": "02174990", "lookup_status": "resolved"}])
...
def test_pdf_only_is_blocked_zip_is_fetched_and_extracted():
    mod = module("fetch_w_3")
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        matrix = root / "matrix.csv"
        ...
        assert report["fetched"] == 1 and report["blocked_or_error"] == 1
        rows = list(csv.DictReader((root / "out" / "europe_accounts_document_fetch_matrix_v2_38w.csv").open(encoding="utf-8")))
        ...
        extracted_path = root / by_ticker["ZIP"]["raw_xhtml_path"].replace("\\", "/") if not Path(by_ticker["ZIP"]["raw_xhtml_path"]).is_absolute() else Path(by_ticker["ZIP"]["raw_xhtml_path"])
...
CASES = [
    ...
    test_pdf_only_is_blocked_zip_is_fetched_and_extracted,
]
...
def main() -> int:
    for case in CASES:
        case()
    print("PASS: v2.38W-accounts-document-fetch/dry-run-gate/pdf-vs-zip-classification/atomic-write/no-credential-leak")
    return 0
