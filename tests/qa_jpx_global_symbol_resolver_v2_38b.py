...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        base = Path(tmp); source = base / "source.csv"; known = base / "known.csv"; overlay = base / "overlay.csv"; runtime = base / "runtime"
        source_rows = [
        ...
        command = [sys.executable, str(SCRIPT), "--limit", "5", "--source", str(source), "--known", str(known), "--overlay", str(overlay), "--runtime", str(runtime)]
        dry = subprocess.run(command, cwd=ROOT, text=True, capture_output=True)
        payload = json.loads(dry.stdout)
        assert dry.returncode == 0 and payload["status"] == "DRY_RUN" and payload["asset_ids"] == ["U00003"]
    ...
    print("PASS: v2.38B JPX-global-resolver/dry-run/resume/exact-match/credential-gate/atomic-overlay/max500/no-phase9c")
    return 0
