...
def main() -> int:
    print("Scout Finance — Phase 7C.2 v2 Stage 2 yfinance policy dry-run checker")
    print("=" * 82)
    ...
    reason_codes = set(reasons_df.get("reason_code", pd.Series(dtype=str)).dropna().astype(str))

    if OLD_BLOCKING_REASON in reason_codes:
        fail(f"Dry-run reasons still contain old blocking {OLD_BLOCKING_REASON}")
        return 1
    ok(f"Old blocking {OLD_BLOCKING_REASON} reason is absent")

    if NEW_PROVIDER_REASON not in reason_codes:
        fail(f"Provider limitation reason is missing: {NEW_PROVIDER_REASON}")
        return 1
    ok(f"Provider limitation reason is present: {NEW_PROVIDER_REASON}")

    if int(summary.get("old_missing_shares_dilution_count", 0)) != 182:
        fail(f"Expected old missing dilution count 182, got {summary.get('old_missing_shares_dilution_count')}")
        return 1
    ok("Old missing dilution count was 182")

    if int(summary.get("provider_limitation_warning_count", 0)) != 182:
        fail(f"Expected provider limitation warning count 182, got {summary.get('provider_limitation_warning_count')}")
        return 1
    ok("Provider limitation warning count is 182")
    ...
    print("Result")
    print("-" * 82)
    ok("Phase 7C.2 v2 Stage 2 yfinance policy dry-run is valid")
    return 0
