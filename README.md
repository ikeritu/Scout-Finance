...
**Fases v2.38K–W — scoring experimental US y fundamentales reales de Europa** (branch `phase9b-global-enrichment-v2-38b`, aún no fusionado a `main`). Sobre la matriz de candidatos US de v2.38J: scoring experimental (v2.38K), shortlist explicada (v2.38L) y contexto macro/geopolítico estático (v2.38M). Fundación de Europa: casa de cotización (v2.38N), precios sin ejecutar (v2.38O/P), enrutamiento de fundamentales (v2.38Q/R/S: 617/55/17), revisión manual de Irlanda (v2.38T, hallazgo `"UKI0"` heredado) y gate de disposición (v2.38U, ninguna ruta lista). **v2.38V**: EODHD exige pago (descartado por política) — en cambio, identidad real de 4/40 activos GB vía OpenFIGI (Rio Tinto, Rentokil, Softcat, Braime) y **3/4 perfiles reales confirmados** en UK Companies House con la credencial gratuita del usuario. **v2.38W — primeros fundamentales reales de Europa**: de esas 3, solo Softcat tiene un documento iXBRL/ESEF real en Companies House (Rio Tinto y Rentokil solo depositan PDF, bloqueadas sin forzar OCR). Extractor real de 14 conceptos IFRS estándar para Softcat: **14/14 extraídos**, con las cuatro ecuaciones contables cuadrando de forma exacta contra los valores reales (activo=pasivo+patrimonio, etc.) — la evidencia más fuerte de que la extracción es correcta. Un bug real de resolución de contexto XBRL (un desglose dimensional de patrimonio competía por el mismo "contexto ganador" que las cifras totales) se encontró y corrigió con una prueba de regresión dedicada antes de aceptar ningún resultado. Ninguna fase de este rango calcula scoring adicional, ranking, recomendaciones ni autoriza fase 9C.
...
- Historial completo, cierre por cierre (v2.33D1 a v2.38W), en `CHANGELOG.md`.
- Extensión global: **9A cerrada; 9B en curso (v2.38C–W cerradas: primeros fundamentales reales de Europa extraídos y verificados contablemente para 1 empresa, 2 bloqueadas por formato de documento); 9C no autorizada**. Este rango (v2.38K–W) vive en el branch `phase9b-global-enrichment-v2-38b` (subido a GitHub, **no fusionado a `main`**). Estado: `outputs/full_universe_source_acquisition/v2_38b_global_enrichment/PHASE9B_EXECUTION_STATUS_v2_38b.md`. El ranking de la app sigue limitado a 50 activos, es experimental y no está validado históricamente.
