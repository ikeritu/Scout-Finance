...
**Fases v2.38K–X — scoring experimental US y fundamentales reales de Europa** (branch `phase9b-global-enrichment-v2-38b`, aún no fusionado a `main`). Sobre la matriz de candidatos US de v2.38J: scoring experimental (v2.38K), shortlist explicada (v2.38L) y contexto macro/geopolítico estático (v2.38M). Fundación de Europa: casa de cotización (v2.38N), precios sin ejecutar (v2.38O/P), enrutamiento de fundamentales (v2.38Q/R/S: 617/55/17), revisión manual de Irlanda (v2.38T, hallazgo `"UKI0"` heredado) y gate de disposición (v2.38U, ninguna ruta lista). **v2.38V**: EODHD exige pago (descartado por política) — identidad real de 4/40 activos GB vía OpenFIGI y **3/4 perfiles reales confirmados** en UK Companies House. **v2.38W**: de esas 3, solo Softcat tiene iXBRL/ESEF real (Rio Tinto y Rentokil solo PDF, bloqueadas sin OCR) — **14/14 conceptos IFRS extraídos**, con las cuatro ecuaciones contables cuadrando de forma exacta (la evidencia más fuerte de que la extracción es correcta); un bug real de contexto XBRL dimensional se encontró y corrigió con prueba de regresión dedicada. **v2.38X — matriz de candidatos**: 9/9 ratios de un solo periodo calculados para Softcat (sin crecimiento interanual todavía); unida a los 0 registros reales de precio de v2.38P, el resultado honesto es **1 único candidato, parcial por falta de precio** — nunca se inventa un valor de precio, solo se documenta por qué falta. Ninguna fase de este rango calcula scoring adicional, ranking, recomendaciones ni autoriza fase 9C.
...
- Extensión global: **9A cerrada; 9B en curso (v2.38C–X cerradas: primeros fundamentales reales de Europa extraídos y verificados contablemente para 1 empresa, unidos en una matriz de candidatos honesta de tamaño 1); 9C no autorizada**. Este rango (v2.38K–X) vive en el branch `phase9b-global-enrichment-v2-38b` (subido a GitHub, **no fusionado a `main`**). Estado: `outputs/full_universe_source_acquisition/v2_38b_global_enrichment/PHASE9B_EXECUTION_STATUS_v2_38b.md`. El ranking de la app sigue limitado a 50 activos, es experimental y no está validado históricamente.
