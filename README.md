...
**v2.38X reconstruido**: la matriz de candidatos ahora combina Softcat y Kingfisher — **2 candidatos, 9/9 ratios cada uno, ambos `CANDIDATE_MATRIX_PARTIAL_PRICE`** (sigue sin haber precio real de Europa). La fila "Softcat" mantiene la advertencia de identidad heredada (ese activo del censo es en realidad SSE PLC; los datos financieros mostrados son reales pero pertenecen a Softcat, no a SSE — corregirlo exige repetir Companies House/iXBRL específicamente para SSE PLC). Ninguna fase de este rango calcula scoring adicional, ranking, recomendaciones ni autoriza fase 9C.
