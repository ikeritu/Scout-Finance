...
**Fases v2.38K–V — scoring experimental US y fundamentales de Europa** (branch `phase9b-global-enrichment-v2-38b`, aún no fusionado a `main`). Sobre la matriz de candidatos US de v2.38J: scoring experimental determinista (v2.38K, 350/555 puntuados, rango 7,5–87,33), shortlist explicada de 50 empresas (v2.38L, 20 alta prioridad) y capa de contexto macro/geopolítico estático (v2.38M, taxonomía, sin noticias en vivo). En paralelo, la fundación de Europa: resolución de casa de cotización (v2.38N, 689/22.578 resueltas), plan de adquisición de precios sin ejecutar (v2.38O), gate de price features fail-closed (v2.38P), enrutamiento + piloto de fundamentales (v2.38Q/R/S: 617/55/17), paquete de revisión manual de Irlanda (v2.38T, hallazgo `company_name="UKI0"` heredado) y el gate de disposición (v2.38U): **ninguna ruta lista** (EODHD exige credencial + script; las otras dos sin identificador/identidad). **v2.38V — primer piloto real**: se confirma que EODHD Fundamentals exige pago (mín. €59,99/mes, choca con la postura del usuario) — no se construye ese runner. En cambio, resolución real de identidad para los 40 activos GB vía OpenFIGI (gratis, sin cuenta): **4/40 resueltos** (Rio Tinto, Rentokil, Softcat, Braime), con dos hallazgos reales aplicados (`exchCode=LN` correcto, no `micCode`; sufijos numéricos espurios en varios tickers, corregibles solo como reintento etiquetado). Localizador de perfil en UK Companies House (API oficial gratuita): el usuario creó su propia cuenta gratuita y guardó `SCOUT_FINANCE_COMPANIES_HOUSE_API_KEY` (nunca vista por este proyecto) — ejecutado de verdad, **3/4 perfiles reales confirmados** (RIO TINTO PLC, SOFTCAT PLC, RENTOKIL INITIAL PLC, con número de empresa y fecha de constitución reales); la cuarta queda sin resolver por un desajuste honesto de nombre, no forzado. Descarga/parseo de cuentas iXBRL queda explícitamente para v2.38W. Ninguna fase de este rango calcula scoring adicional, ranking, recomendaciones ni autoriza fase 9C.
...
- Historial completo, cierre por cierre (v2.33D1 a v2.38V), en `CHANGELOG.md`.
- Extensión global: **9A cerrada; 9B en curso (v2.38C–V cerradas, hasta el primer piloto real de fundamentales de Europa: 4/40 identidades GB resueltas, 3/4 perfiles reales confirmados en Companies House); 9C no autorizada**. Este rango (v2.38K–V) vive en el branch `phase9b-global-enrichment-v2-38b` (subido a GitHub, **no fusionado a `main`**). Estado: `outputs/full_universe_source_acquisition/v2_38b_global_enrichment/PHASE9B_EXECUTION_STATUS_v2_38b.md`. El ranking de la app sigue limitado a 50 activos, es experimental y no está validado históricamente.
