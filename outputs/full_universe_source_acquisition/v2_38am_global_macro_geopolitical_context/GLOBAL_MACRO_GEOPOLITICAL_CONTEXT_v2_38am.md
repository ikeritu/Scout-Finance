...
## Decimotercera reconstrucción (2026-09-07): segunda mitad de "consolidar primero" — de 1.244 a 5.034 empresas

Instrucción del usuario tras completar Luxemburgo/Austria/Finlandia y decidir explícitamente "Consolidar primero" (frente a abrir Reino Unido/EE. UU. o los cierres offshore): reconstruir también este módulo, ya que `v2.38AL` (la matriz de cobertura de la que este módulo lee) acababa de pasar de 1.244 a **5.034** empresas con identidad real. Ningún script de recolección nuevo — se conecta lo ya construido, igual que en la reconstrucción de `v2.38AL` que la precedió el mismo día.

### Qué se conecta

- **Automático, sin cambio de ruta**: `COVERAGE_INPUT` ya apuntaba al fichero que `v2.38AL` reescribe — las 3.790 empresas nuevas de Luxemburgo, Austria y la resolución masiva de Cboe Europe (54 países) llegan aquí sin tocar el script, simplemente porque la fuente de la que lee ya creció.
- **Finlandia (`v2.38BH`)**: las 127 empresas nuevas de la extensión de Cboe Europe reutilizaron el código de `v2.38AU` pero escribieron su resultado en un fichero separado — se conecta como una segunda fuente de texto de Finlandia (`FINLAND_BH_EXTENSION_INPUT`), con el mismo campo `tol_description_en` ya en inglés real, sin tabla de traducción nueva.
- **Luxemburgo y Austria (empresas nuevas)**: ninguna fuente de texto narrativo existe todavía para ellas — se quedan honestamente en emparejamiento solo por `company_name`, igual que cualquier otra empresa europea sin fuente dedicada; no se inventó texto para rellenar el hueco.
- **`EU_MEMBER_COUNTRIES`/`EUROZONE_COUNTRIES` ampliados**: de 11/9 países (el alcance original de 13 países europeos) a 22/15, añadiendo miembros reales y ya consolidados que aparecen entre los 54 países de la resolución masiva de Cboe Europe — Luxemburgo (UE+Eurozona), Polonia, Portugal (UE+Eurozona), Chequia, Hungría, Rumanía, Bulgaria, Chipre (UE+Eurozona), Estonia (UE+Eurozona), Lituania (UE+Eurozona) y Malta (UE+Eurozona). Cada adopción del euro y cada adhesión a la UE añadida es un hecho bien documentado y muy anterior a la fecha de referencia del proyecto, siguiendo la misma disciplina de "solo hechos estables y no controvertidos" que ya regía la lista original.

### Resultado real

**5.034/5.034 empresas procesadas, 0 rechazadas** (frente a 1.244/1.244 antes). `MACRO_CONTEXT_READY` sube de **168 a 700**; `MACRO_CONTEXT_PARTIAL` pasa a 4.334. Desglose de `sector_text_source_counts`: `v2.38J` (EE. UU.) 555 sin cambios, `v2.38AN` (GB) 29, `v2.38AO` (Francia) 18, `v2.38AQ` (Países Bajos) 44, `v2.38AR` (Wikidata generalizado) 97, `v2.38AS` (Austria original) 5, `v2.38AT` (Irlanda) 3, `v2.38AU` (Finlandia original) 5, **`v2.38BH` (Finlandia extensión) 127 — nueva** — y **4.151 sin ninguna fuente narrativa**, correctamente en emparejamiento solo por nombre.

La mayor parte de la subida en `MACRO_CONTEXT_READY` no viene de texto narrativo nuevo, sino de que la resolución masiva de Cboe Europe (3.766 empresas reales en 54 países) trajo consigo compañías cuyo propio `company_name` ya contiene una palabra clave de sector (bancos, aerolíneas, energéticas, farmacéuticas, etc.) — un resultado honesto y esperado dado el tamaño de la población nueva, no una mejora de la fuente de datos en sí. Los 4.151 sin fuente y sin coincidencia por nombre quedan correctamente marcados `MACRO_CONTEXT_PARTIAL`, con la limitación explícita en `macro_limitations` de cada fila.

**Estado del bloque (decimotercera reconstrucción): `COMPLETED_GLOBAL_MACRO_GEOPOLITICAL_CONTEXT_STATIC_NOT_RECOMMENDATIONS`.** 4 pruebas offline nuevas añadidas (22 en total): fuente de Finlandia-extensión atribuida correctamente a `v2.38BH` (y su caso no-resuelto), Luxemburgo con temas UE+Eurozona, Bulgaria con UE pero sin Eurozona. Con esto se cierra "consolidar primero" — el siguiente paso, ya identificado, es decidir si abrir Reino Unido/EE. UU. vía Companies House/SEC o cerrar primero las 6 jurisdicciones offshore sin divulgación pública.
