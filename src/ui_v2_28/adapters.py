...
MAINTENANCE_REPORT="outputs/full_universe_source_acquisition/v2_26f_scheduled_maintenance_plan/maintenance_plan_report.json"
...
def load_universe(root:Path,pointer_rel=UNIVERSE_POINTER,verify_hash=True):
 ...
 try:
  ...
  if verify_hash and expected and sha256(dataset)!=expected:raise ValueError("operational universe SHA-256 mismatch")
  ...
  errors.append(str(e));return UniverseState(False,status="UNAVAILABLE_FAIL_CLOSED",errors=tuple(errors))

def scoring_checks(p):
 c=p.get("consumer_contract") or {}
 return {
  "schema_version_supported":p.get("schema_version")=="1.0",
  "status_is_active":p.get("status") not in (None,"NO_ACTIVE_PRODUCTION_SCORING_FAIL_CLOSED"),
  "active_scoring_available_true":p.get("active_scoring_available") is True,
  "production_scoring_authorized_true":p.get("production_scoring_authorized") is True,
  "scoring_promoted_true":p.get("scoring_promoted") is True,
  "active_scoring_artifact_non_null":bool(p.get("active_scoring_artifact")),
  "active_scoring_sha256_present":bool(p.get("active_scoring_sha256")),
  "allow_ranking_true":c.get("allow_ranking") is True,
 }
...
def load_scoring(root:Path,pointer_rel=SCORING_POINTER):
 ...
 try:
  ...
  if not all(checks.values()):
   ...
   return ScoringState(ConsumerState.SCORING_UNAVAILABLE,p.get("status","SCORING_UNAVAILABLE"),False,diagnostic_rows=int(diagnostic.get("rows",0)),errors=failed)
  ...
  if sha256(artifact)!=p["active_scoring_sha256"]:raise ValueError("active scoring SHA-256 mismatch")
  ...
  return ScoringState(ConsumerState.SCORING_UNAVAILABLE,"SCORING_UNAVAILABLE_FAIL_CLOSED",False,errors=(str(e),))
...
def build_app_state(root:Path,verify_universe_hash=True):
 root=root.resolve()
 return AppState(root,load_universe(root,verify_hash=verify_universe_hash),load_scoring(root),load_maintenance(root))
