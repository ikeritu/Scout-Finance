...
# 7E.1: do not require the obsolete original 7D marker.
# 7D.1 intentionally removed that bad early render block and replaced it with a safe hotfix marker.
REQUIRED_APP_TEXT = [
...
def render_report(summary: dict) -> str:
    ...
    marker_lines = "\n".join(
        f"- `{marker}`: {present}"
        for marker, present in summary["required_app_text_checks"].items()
    )
    optional_lines = "\n".join(
        f"- `{marker}`: {present}"
        for marker, present in summary["optional_legacy_app_text_checks"].items()
    )

    return f"""# Scout Finance — Phase 7E.1 v0.7 release checkpoint marker fix
...
def main() -> int:
    print("Scout Finance — Phase 7E.1 v0.7 release checkpoint marker fix")
    print("=" * 92)
    ...
    required_checks = {marker: marker in app_text for marker in REQUIRED_APP_TEXT}
    optional_checks = {marker: marker in app_text for marker in OPTIONAL_LEGACY_APP_TEXT}
    ...
    d1 = read_json(REQUIRED_FILES["phase7d1_hotfix"])
    ...
    checks = {
        ...
        "required_dashboard_markers_ok": all(required_checks.values()),
        "required_files_ok": all(item["exists"] for item in evidence_files),
        "stage2_policy_ok": stage2_impl.get("status") == "OK",
        "dashboard_hotfixes_ok": d1.get("status") == "OK" and d2.get("status") == "OK" and d3b.get("status") == "OK",
    ...
    summary = {
        ...
        "recommended_next_phase": "7F — Package release v0.7 candidate" if ready else "7E.2 — Fix remaining checkpoint evidence before release packaging",
        ...
        "required_app_text_checks": required_checks,
        "optional_legacy_app_text_checks": optional_checks,
