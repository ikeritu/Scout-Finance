#!/usr/bin/env python3
"""Block 9AA, part 1: resolve real company identity for the 15 Spain
(Bolsa de Madrid, MIC XMAD) assets flagged in v2.38S with the exact same
unverified placeholder already fixed for GB (v2.38V) and Ireland (v2.38Z):
company_name="ESP0" (Xetra's "Product Assignment Group" market-segment
code for Spain, mis-mapped to company_name in the same ancient ingestion
phase), and a "ticker" field that is really Deutsche Boerse Xetra's own
internal cross-listing mnemonic, not a real Bolsa de Madrid ticker.

Naming note: this is the third jurisdiction fixed with this method after
GB (v2.38V, letter-lettered blocks ran out at Z), so this block continues
the sequence as "AA" -- the same convention spreadsheets use once letters
run out (…, Y, Z, AA, AB, …).

Same fix as resolve_europe_gb_identity_xetra_source_v2_38v.py and
resolve_europe_ireland_identity_xetra_source_v2_38z.py: resolve directly
...
OUT = ROOT / "outputs/full_universe_source_acquisition/v2_38aa_europe_spain_identity_resolution"
PHASE = "v2.38AA-europe-spain-identity-resolution-xetra-source"
JURISDICTION_FILTER = "ES"
...
def clean_company_name(raw_instrument: str) -> str:
    ...
    every instrument name (e.g. 'IBERDROLA SA EO-,75' -> 'IBERDROLA SA').
...
def build(input_matrix: Path, xetra_raw: Path, output_dir: Path) -> dict[str, Any]:
    ...
    report = {
        ...
        "same_method_as": "resolve_europe_gb_identity_xetra_source_v2_38v.py / resolve_europe_ireland_identity_xetra_source_v2_38z.py",
        "phase9c_authorized": False,
    }

    output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(output_dir / "europe_spain_identity_resolution_xetra_source_matrix_v2_38aa.csv", matrix, MATRIX_FIELDS)
    write_text(output_dir / "europe_spain_identity_resolution_xetra_source_summary_v2_38aa.json", json.dumps(report, indent=2, sort_keys=True) + "\n")
    ...
    manifest = {
        ...
        "outputs": {}, "scripts": ["scripts/resolve_europe_spain_identity_xetra_source_v2_38aa.py"],
    ...
    for path in sorted(output_dir.glob("europe_spain_identity_resolution_xetra_source_*")):
        if path.name != "europe_spain_identity_resolution_xetra_source_manifest_v2_38aa.json":
            manifest["outputs"][path.name] = {"bytes": path.stat().st_size, "sha256": sha(path)}
    write_text(output_dir / "europe_spain_identity_resolution_xetra_source_manifest_v2_38aa.json", json.dumps(manifest, indent=2, sort_keys=True) + "\n")
    return report
