#!/usr/bin/env python3
"""Block 9Y, part 2: for each of the (up to 40) real companies confirmed
in part 1 (run_europe_companies_house_lookup_v2_38y.py), find the most
recent "accounts" filing and check what format Companies House actually
holds for it. Only a package containing a real inline-XBRL (iXBRL/ESEF)
document is downloaded and extracted -- PDF-only accounts are recorded as
blocked with an explicit reason, never OCR'd or guessed at.

Identical logic to fetch_europe_accounts_documents_v2_38w.py (v2.38W),
run again against the full 40-company lookup matrix produced after the
v2.38V identity correction, instead of the original 3-4 identities.

Blocked by default. --execute plus SCOUT_FINANCE_COMPANIES_HOUSE_API_KEY
are both required -- the same credential v2.38V/W/Y already use, never
read into anything beyond the HTTP Basic auth header, never logged.
"""
...
PHASE = "v2.38Y-europe-accounts-document-fetch"
...
def build(input_matrix: Path, output_dir: Path, raw_cache: Path, api_key: str) -> dict[str, Any]:
    ...
    report = {
        ...
        "fetch_reason_counts": {reason: sum(1 for r in matrix if r["fetch_reason"] == reason) for reason in sorted({r["fetch_reason"] for r in matrix})},
    }
    output_dir.mkdir(parents=True, exist_ok=True)
    write_csv(output_dir / "europe_accounts_document_fetch_matrix_v2_38y.csv", matrix, MATRIX_FIELDS)
    write_text(output_dir / "europe_accounts_document_fetch_summary_v2_38y.json", json.dumps(report, indent=2, sort_keys=True) + "\n")
    return report
