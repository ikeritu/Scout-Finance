"""Block 9Y, part 3: parse the real iXBRL/ESEF report(s) fetched by
fetch_europe_accounts_documents_v2_38y.py into structured fundamental
records for a fixed, small set of standard ifrs-full:* concepts.

Identical extraction logic to normalize_europe_ixbrl_fundamentals_v2_38w.py
(v2.38W) -- same targeted, non-dimensional-context-only parser, same
number-cleaning rules -- run again against however many of the (up to 40)
GB companies confirmed after the v2.38V identity correction actually have
a real iXBRL package available at Companies House (most large PLCs only
file PDF; see v2.38W's own findings for Rio Tinto/Rentokil).
"""
...
PHASE = "v2.38Y-europe-ixbrl-fundamentals"
...
def load_contexts(root) -> dict[str, dict[str, str]]:
    ...
    for ctx in root.iter(f"{{{NS['xbrli']}}}context"):
        ...
        # equity -- rather than the undimensioned "total" line item real
        # filings also tag for the same date. Only the plain context is
        # the total we want (see v2.38W for the real bug this guards).
...
def build(fetch_matrix: Path, output_dir: Path, records_output: Path) -> dict[str, Any]:
    ...
    report = {
        "phase": PHASE, "companies_with_fetched_document": len(rows),
    ...
    tmp2 = (output_dir / "europe_ixbrl_fundamentals_coverage_report_v2_38y.json").with_suffix(".json.tmp")
    tmp2.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    tmp2.replace(output_dir / "europe_ixbrl_fundamentals_coverage_report_v2_38y.json")
    return report
...
def main() -> int:
    ...
    parser.add_argument("--fetch-matrix", type=Path, default=ROOT / "outputs/full_universe_source_acquisition/v2_38y_europe_gb_full_expansion/europe_accounts_document_fetch_matrix_v2_38y.csv")
